...
def create_company_card(profile_info):
    ...
    return html.Div([
        ...
        html.Ul([
            html.Li(f"Industry: {profile_info['產業']} / {profile_info['產業別']}"),
            html.Li(f"Page: {profile_info['網站']}"),
            html.Li(f"Address: {profile_info['地址']}"),
            html.Li(f"Tel: {profile_info['電話']}"),
            html.Li(f"Amount of employee: {profile_info['員工人數']}"),
            html.Li(f"Market value: {profile_info['市值']}"),
            html.Li(f"Total revenue: {profile_info['營收']}"),
            html.Li(f"Profit: {profile_info['淨利']}"),
            html.Li(f"Price-to-Earnings (PE): {profile_info['本益比 (PE)']}"),
            html.Li(f"Dividend yield: {profile_info['股息殖利率']}"),
            html.Li(f"Beta值: {profile_info['Beta值']}"),
            html.Li(f"Analyst Rating: {profile_info['分析師評價']}"),
